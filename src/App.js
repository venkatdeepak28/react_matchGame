...
let shuffledList = [...imagesList]

shuffledList.sort(() => 0.5 - Math.random())

class App extends Component {
  state = {count: 0, timer: 60, isWin: false, arrValue: []}
  ...
  checkObj = id => {
    if (presentObj[0].id === id) {
      shuffledList = shuffledList.filter(eachValue => eachValue.id !== id)
      this.setState(prevState => ({
        count: prevState.count + 1,
        arrValue: [...prevState.arrValue, id],
      }))
    } else {
      shuffledList = imagesList.sort(() => Math.random() - 0.5)
  ...
  tick = () => {
    ...
    if (timer <= 1) {
      clearInterval(intervalId)
      this.setState({timer: 60, isWin: true})
    } else {
      this.setState(prevState => ({
  ...
  render() {
    const {count, timer, isWin} = this.state

    presentObj = shuffledList.slice(0, 1)
